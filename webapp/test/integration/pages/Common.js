sap.ui.define([
		...
		function getFrameUrl (sHash, sUrlParameters) {
			var sUrl = jQuery.sap.getResourcePath("qldh/mypaysumm/app", ".html");
			sHash = sHash || "";
			sUrlParameters = sUrlParameters ? "?" + sUrlParameters : "";

			if (sHash) {
				sHash = "#MyPaymentSummaries-display&/" + (sHash.indexOf("/") === 0 ? sHash.substring(1) : sHash);
			} else {
				sHash = "#MyPaymentSummaries-display";
			}

			return sUrl + sUrlParameters + sHash;
		}

		return Opa5.extend("qldh.mypaysumm.test.integration.pages.Common", {
			...
			getMockServer : function () {
				return new Promise(function (success) {
					Opa5.getWindow().sap.ui.require(["qldh/mypaysumm/localService/mockserver"], function (mockserver) {
