			...
			onTheMasterPage : {
				...
				actions : {
							...
							success : function (aEntityData) {
								this.waitFor({
									...
									success : function (oList) {
										var sCurrentId,
											aItemsNotInTheList = aEntityData.filter(function (oObject) {
												return !oList.getItems().some(function (oListItem) {
													return oListItem.getBindingContext().getProperty("Financialyr") === oObject.Financialyr;
												});
											});

										if (!aItemsNotInTheList.length) {
											// Not enough items all of them are displayed so we take the last one
											sCurrentId = aEntityData[aEntityData.length - 1].Financialyr;
										} else {
											sCurrentId = aItemsNotInTheList[0].Financialyr;
										}
										...
										oCurrentItem.bindingPath = "/" + oList.getModel().createKey("PeriodsSet", {
											Financialyr : sCurrentId
										});
					...
					iRememberTheListItem : function (oListItem) {
						...
						this.getContext().currentItem = {
							...
							id: oBindingContext.getProperty("Financialyr"),
							title: oBindingContext.getProperty("Financialyr")
						};
					}
				},
				...
				assertions : {
					...
					theListShouldBeSortedAscendingOnName : function () {
						return this.theListShouldBeSortedAscendingOnField("Financialyr");
					},
					...
					theListShouldHaveAllEntries : function () {
						...
						// retrieve all PeriodsSet to be able to check for the total amount
